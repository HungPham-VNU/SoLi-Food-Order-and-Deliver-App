...
const catalogUrl = process.env.CATALOG_DATABASE_URL || 'postgresql://uitfood_catalog:catalog_secret@localhost:5432/uitfood_catalog';
const orderingUrl = process.env.ORDERING_DATABASE_URL || 'postgresql://uitfood_ordering:ordering_secret@localhost:5432/uitfood_ordering';
...
async function sync() {
  ...
  for (const r of restaurants) {
    await orderingDb.insert(orderingSchema.orderingRestaurantSnapshots).values({
      restaurantId: r.id,
      name: r.name,
  ...
  for (const z of zones) {
    await orderingDb.insert(orderingSchema.orderingDeliveryZoneSnapshots).values({
      zoneId: z.id,
  ...
  for (const item of items) {
    ...
    const groups = await catalogDb.select().from(catalogSchema.modifierGroups)
      .where(sql`${catalogSchema.modifierGroups.menuItemId} = ${item.id}`);
    
    const modifiers = [];
    for (const g of groups) {
      const options = await catalogDb.select().from(catalogSchema.modifierOptions)
    ...
    await orderingDb.insert(orderingSchema.orderingMenuItemSnapshots).values({
      menuItemId: item.id,
