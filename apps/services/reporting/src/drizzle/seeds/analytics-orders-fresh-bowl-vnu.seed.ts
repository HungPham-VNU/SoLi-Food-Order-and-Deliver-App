...
async function seedAnalyticsOrdersFreshBowlVnu() {
  try {
    ...
    const { rows: restaurants } = await catalogPool.query("SELECT id, name FROM restaurants WHERE name = 'Fresh Bowl VNU'");
    
    if (restaurants.length === 0) {
      console.log('❌ Fresh Bowl VNU restaurant not found. Please run catalog seeds first.');
      return;
    }
    const targetRestaurant = restaurants[0];

    const { rows: menuItems } = await catalogPool.query('SELECT id, name, price FROM menu_items WHERE restaurant_id = $1', [targetRestaurant.id]);
    if (menuItems.length === 0) {
      console.log('❌ No menu items found for Fresh Bowl VNU.');
      return;
    }

    console.log(`🍽️  Found Fresh Bowl VNU with ${menuItems.length} menu items. Creating analytics orders...`);
    const totalOrders = 150;
    
    // Seed restaurant facts first
    await db.insert(schema.reportingRestaurantFacts).values({
...
void seedAnalyticsOrdersFreshBowlVnu().then(async () => { await catalogPool.end(); process.exit(0); });
