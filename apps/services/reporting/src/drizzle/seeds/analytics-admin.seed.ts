...
async function seedAnalyticsOrdersAdmin() {
  try {
    ...
    console.log(
      `🍽️  Found ${validRestaurants.length} valid restaurants. Creating analytics...`,
    );
    const totalOrders = 300;

    // Seed restaurant facts first
    const restaurantFacts: schema.NewReportingRestaurantFact[] =
...
void seedAnalyticsOrdersAdmin().then(async () => {
  await catalogPool.end();
  process.exit(0);
});
